...
for first in range(len(df['жалобы'].unique()) - 1):
    if df['жалобы'][first] not in m:
        m.append(df['жалобы'][first])
        for i in m:
            if similarity(i, df['жалобы'][first]) > 0.55:
                df['жалобы'] = df['жалобы'].replace(df['жалобы'][first], i)
                break
...
df.to_csv(r'finally_data.csv', index=False)